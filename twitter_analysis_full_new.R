...
#                                                                                               #
# Script names  : twitter_mining_analysis.R                                                     #
...
wd <- 'D:/Gdrive/ITB/01_Twitter_data_crawling_and_manipulation/05_working_dir'
setwd(wd)
...
terms <- c("prayforjayapura","#prayforsentani","sentani")
...
tweets.by.terms <- searchTwitter(searchString = paste0(search.terms," -filter:retweets"), n=3200,
                                 resultType='recent')
...
tw.corpus <- Corpus(VectorSource(tweets.df$text))
...
tw.corpus.proc <- tm_map(tw.corpus.proc, removeWords, c(terms,"gempa","tsunami","sulteng","palu","amp","donggala","sigi","sulawesi","sulsel"))
...
set.seed(1234)
wordcloud(tw.corpus.proc,min.freq=10,max.words=80,scale=c(3,.4), colors=brewer.pal(8, "Dark2"),random.color=T, random.order=F)
...
#load map data
ina.shp.2012 <- readOGR(dsn = "D:/Gdrive/ITB/01_Twitter_data_crawling_and_manipulation/01_input/Ina_map_2015", 
                        layer = 'AdminKabKota2015')
